...
/**
 * Liefert für jedes über den kanonischen Prädikatknoten konfigurierte Axiom eine
 * Definitionskarte. Da die Karte direkt aus [AxiomOperatorDefinition] erzeugt
 * wird, erhalten neu registrierte Axiome denselben Definitionsdialog automatisch.
 */
internal fun axiomPraedikatKonzept(knoten: KnotenDaten): KonzeptDefinition? {
    ...
    if (knoten.parameter[PRAEDIKAT_SEITE_PARAMETER] != PRAEDIKAT_SEITE_AXIOME) return null
    ...
    val systeme = definition.systeme.sorted()
    val systemText = systeme.joinToString()

    return KonzeptDefinition(
        id = KonzeptId("axiom-praedikat-${definition.stabileId.removePrefix("axiom.").replace('.', '-')}"),
        name = "Axiom: ${definition.titel}",
        beschreibung = buildString {
            append("Definitionskarte des Axiom-Prädikats ")
            append(definition.titel)
            append(". Die Eingänge entsprechen den Parametern des Axioms; der Ausgang ist die daraus gebildete Aussage.")
        ...
        pfad = listOf("Aussagen", "Prädikate", "Axiome", definition.kategorie),
...
private fun axiomPraedikatDefinitionsKarte(definition: AxiomOperatorDefinition): KartenDaten {
    val suffix = definition.stabileId.removePrefix("axiom.").replace('.', '-')
    val prefix = "definition-axiom-$suffix"
    ...
    val regel = KnotenDaten(
        ...
        parameter = mapOf(
            ...
            "axiomensysteme" to definition.systeme.sorted().joinToString(),
    ...
    val ausgangsKnoten = axiomAusgang(prefix, ausgang, eingänge.size)
...
private fun axiomEingang(
    prefix: String,
    anschluss: AnschlussDaten,
    argument: AxiomArgument,
    index: Int,
): KnotenDaten {
    ...
    return KnotenDaten(
        ...
        parameter = mapOf(
            "typ" to axiomArgumentTyp(argument),
...
private fun axiomArgumentTyp(argument: AxiomArgument): String = when (argument.art) {
    AxiomArgumentArt.OBJEKT -> "Mathematisches Objekt"
    AxiomArgumentArt.MENGE -> "Menge"
    AxiomArgumentArt.METHODE -> argument.stelligkeit
        ?.let { "Methode mit $it Argumenten" }
        ?: "Methode"
    AxiomArgumentArt.PRAEDIKAT -> argument.stelligkeit
        ?.let { "$it-stelliges Prädikat" }
        ?: "Prädikat"
}
