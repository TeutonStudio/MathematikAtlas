...
internal object SkalarproduktKartenQuelle {
    ...
    fun kandidaten(): List<SkalarproduktZertifikatsKarte> {
...
internal fun alsSkalarproduktZertifikatsKarte(karte: KartenDaten): SkalarproduktZertifikatsKarte {
    ...
    if (begriff.parameter[SKALARPRODUKT_ZERTIFIKAT_VERSION_PARAMETER]?.toIntOrNull() !=
...
internal object SkalarproduktVektorRechnerInspektor : KnotenInspektor {
    @Composable
    override fun Inhalt(
        knoten: KnotenDaten,
        ...
        ExposedDropdownMenuBox(
            expanded = operatorGeöffnet,
            onExpandedChange = { operatorGeöffnet = it },
        ) {
            OutlinedTextField(
            ...
            ExposedDropdownMenu(
                expanded = operatorGeöffnet,
                onDismissRequest = { operatorGeöffnet = false },
            ) {
                DropdownMenuItem(
                    ...
                    onClick = {
                        ...
                        aktionen.parameter(
                            VEKTOR_RECHNER_OPERATOR,
                            VektorRechnerOperator.SKALARPRODUKT.stabileId,
                        )
                    },
                )
            }
        ...
        Text(
            "Zeilenvektoren, Spaltenvektoren und kartesische Zahlentupel sind zulässig; Orientierung und Komponentenreihenfolge bleiben erhalten.",
...
private fun SkalarproduktDefinitionAuswahl(
    ...
    val kandidaten = SkalarproduktKartenQuelle.kandidaten()
    ...
    ExposedDropdownMenuBox(expanded = geöffnet, onExpandedChange = { geöffnet = it }) {
        OutlinedTextField(
            value = if (istStandard) {
                "Standardskalarprodukt"
            } else {
                "${ausgewaehlt?.name} · v${ausgewaehlt?.verweis?.version}"
            },
            onValueChange = {},
            readOnly = true,
...
internal object SkalarproduktBegriffInspektor : KnotenInspektor {
    ...
    override fun Inhalt(
        ...
        ExposedDropdownMenuBox(
            expanded = bereichGeöffnet,
            onExpandedChange = { bereichGeöffnet = it },
        ) {
            OutlinedTextField(
            ...
            ExposedDropdownMenu(
                expanded = bereichGeöffnet,
                onDismissRequest = { bereichGeöffnet = false },
            ) {
                listOf(
