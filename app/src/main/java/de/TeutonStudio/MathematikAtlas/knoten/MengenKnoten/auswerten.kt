...
typealias MengenAuswertenDaten = auswerten.MengenAuswertenDaten

class auswerten(
    override val graph: Graph,
    override val daten: MengenAuswertenDaten,
    override val besitzer: GraphDatenObjektKarte<*>,
) : GraphDatenObjektKnoten<MengenAuswertenDaten>, GraphDatenObjektInspektor<MengenAuswertenDaten> {
    ...
    class MengenAuswertenDaten(
        override val id: GraphDatenId,
        override val name: String = "Menge auswerten",
    ) : GraphDatenKnoten {
        ...
        init {
            val eingang = MengenObjektAnschluss.MengenAnschlussDaten("$id-eingang-0", Kante.Links, Richtung.Eingang).apply {
                label = "Menge"
        ...
        fun formel(): String =
            (anschlüsse.firstOrNull() as? MengenObjektAnschluss.MengenAnschlussDaten)
                ?.let { it.cache as? MengenObjektAnschluss.MengenAnschlussDaten.CacheDaten }
                ?.latex
    ...
    companion object {
        const val KNOTEN_ART: KnotenArt = "auswertenMenge"
    }
}
