...
class KoordinatenAdapterTest {
    ...
    @Test
    fun `skalar wird nur als eindimensionale reelle Koordinate behandelt`() {
        val eindimensional = assertIs<KoordinatenErgebnis.Darstellbar>(
            KoordinatenAdapter.extrahiere(RationaleZahl.von(7), 1),
        )
        val zweidimensional = assertIs<KoordinatenErgebnis.NichtDarstellbar>(
            KoordinatenAdapter.extrahiere(RationaleZahl.von(7), 2),
        )

        assertEquals(listOf(7.0), eindimensional.werte)
        assertTrue("nur in R¹" in zweidimensional.grund)
    }
    ...
    @Test
    fun `reell eingebettete komplexe Zahl bleibt reelle Koordinate`() {
        val ergebnis = assertIs<KoordinatenErgebnis.Darstellbar>(
        ...
        assertEquals(listOf(3.0), ergebnis.werte)
    }

    @Test
    fun `echte komplexe Zahl verlangt ausdrueckliche Projektion`() {
        val ergebnis = assertIs<KoordinatenErgebnis.ProjektionErforderlich>(
            KoordinatenAdapter.extrahiere(
        ...
        assertTrue("komplexe Zahl" in ergebnis.grund)
    }
