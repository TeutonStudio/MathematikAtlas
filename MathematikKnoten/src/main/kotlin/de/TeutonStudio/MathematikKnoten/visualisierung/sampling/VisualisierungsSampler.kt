...
private fun numerisch(ausdruck: ZahlAusdruck?, werte: Map<String, Double>): Double? = when (ausdruck) { null -> null; is RationaleZahl -> ausdruck.zuDezimal(16).toDouble(); is Variable -> werte[ausdruck.name]; is Addition -> ausdruck.summanden.map { numerisch(it, werte) }.takeUnless { it.any { n -> n == null } }?.sumOf { it!! }; is Multiplikation -> ausdruck.faktoren.map { numerisch(it, werte) }.takeUnless { it.any { n -> n == null } }?.fold(1.0) { acc, n -> acc * n!! }; is Division -> numerisch(ausdruck.dividend, werte)?.let { l -> numerisch(ausdruck.divisor, werte)?.takeIf { abs(it) > 1e-12 }?.let { l / it } }; is Potenz -> numerisch(ausdruck.basis, werte)?.let { b -> numerisch(ausdruck.exponent, werte)?.let { b.pow(it) } }; is Betrag -> numerisch(ausdruck.argument, werte)?.let(::abs); is Sinus -> numerisch(ausdruck.argument, werte)?.let(::sin); is Cosinus -> numerisch(ausdruck.argument, werte)?.let(::cos); is Exponentialfunktion -> numerisch(ausdruck.argument, werte)?.let(::exp); is NatürlicherLogarithmus -> numerisch(ausdruck.argument, werte)?.takeIf { it > 0 }?.let(::ln); is Wurzel -> numerisch(ausdruck.argument, werte)?.takeIf { it >= 0 }?.let(::sqrt); else -> null }
