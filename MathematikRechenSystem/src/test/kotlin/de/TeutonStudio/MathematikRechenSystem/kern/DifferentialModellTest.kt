...
class DifferentialModellTest {
    ...
    @Test fun `erste Ordnung rendert f Strich und hoehere Ordnungen bleiben roemisch`() { val m=quadratischeMethode(); assertEquals("f'",totaleAbleitung(m,DifferentialOrdnung.Konkret(1)).zuLatex()); assertEquals("f^{\\mathrm{II}}",totaleAbleitung(m,DifferentialOrdnung.Konkret(2)).zuLatex()); assertEquals("f^{\\mathrm{IV}}",totaleAbleitung(m,DifferentialOrdnung.Konkret(4)).zuLatex()); val n=differenziereMethodeStrukturiert(m,DifferentialOrdnung.Konkret(0)); assertEquals(m,n.methode); assertEquals(DifferentialUnterstuetzungsStatus.BERECHNET,n.status) }
