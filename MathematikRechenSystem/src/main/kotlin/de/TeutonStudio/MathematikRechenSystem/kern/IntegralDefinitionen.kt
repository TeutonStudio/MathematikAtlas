...
object IntegralDefinitionsKatalog {
    ...
    val mass = ImpliziteDefinition(
        ...
        existenzStatus = NachweisStatus.Bedingt(listOf(UnentscheidbareAussage("geeignetes Maß", "Integralvertrag"))),
        eindeutigkeitsStatus = NachweisStatus.Bedingt(listOf(UnentscheidbareAussage("Maß eindeutig aus Bereich ableitbar", "Integralvertrag"))),
    ...
    val riemann = ImpliziteDefinition(
        ...
        charakterisierendeRegeln = listOf(
            DefinitionsRegel(id = "integral.riemann.domain", name = "Kartesisches Produkt beschränkter Intervalle", folgerungLatex = "M=I_1\\times\\cdots\\times I_n"),
            DefinitionsRegel(id = "integral.riemann.hauptsatz", name = "Eindimensionaler Hauptsatz", folgerungLatex = "\\int_{[a,b]}f\\cdot dx=F(b)-F(a),\\;F^{\\mathrm I}=f"),
        ),
        existenzStatus = NachweisStatus.Bedingt(listOf(UnentscheidbareAussage("Riemann-Integrierbarkeit", "Riemann-Integral"))),
        eindeutigkeitsStatus = NachweisStatus.Nachgewiesen,
    ...
    val zaehlmass = ExpliziteDefinition(
        ...
        wert = AllgemeinerParameter(name = "zaehlIntegral", latex = "\\int_M f\\,d\\#=\\sum_{x\\in M}f(x)"),
        voraussetzungen = listOf(UnentscheidbareAussage("endlicher oder summierbarer Bereich", "Zählmaß")),
        referenzen = setOf(operator.id, mass.id),
    )
    ...
    val nichtstandard = ImpliziteDefinition(
        ...
        charakterisierendeRegeln = listOf(
            DefinitionsRegel(id = "integral.nichtstandard.summe", name = "Hyperendliche Summe", folgerungLatex = "\\operatorname{st}\\left(\\sum_{i=1}^{H}{}^*f(\\xi_i)\\cdot\\Delta x_i\\right)"),
            DefinitionsRegel(id = "integral.nichtstandard.unabhaengig", name = "Unabhängigkeit von Stützstellen", folgerungLatex = "\\operatorname{st}(S_H)\\text{ ist unabhängig von der zulässigen Wahl der }\\xi_i"),
        ),
        existenzStatus = NachweisStatus.Bedingt(listOf(UnentscheidbareAussage("hyperendliche Partition", "Nichtstandardintegral"), UnentscheidbareAussage("Standardteil existiert", "Nichtstandardintegral"), UnentscheidbareAussage("Unabhängigkeit von Stützstellen", "Nichtstandardintegral"))),
        eindeutigkeitsStatus = NachweisStatus.Bedingt(listOf(UnentscheidbareAussage("eindeutiger Standardteil", "Nichtstandardintegral"))),
        referenzen = setOf(operator.id, mass.id),
    )
    ...
    val klassischeGrenzen = ExpliziteDefinition(
        ...
        wert = AllgemeinerParameter(name = "grenzen", latex = "\\int_a^b f(x)\\,dx\\equiv\\int_{x\\in[a,b]}f(x)\\cdot dx"),
        referenzen = setOf(termForm.id, bereich.id),
    )

    val alle: List<MathematischeDefinition> = listOf(operator, methodenForm, termForm, bindung, bereich, mass, riemann, zaehlmass, nichtstandard, klassischeGrenzen)
